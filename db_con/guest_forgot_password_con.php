...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $sql = "SELECT status FROM guest_password_reset_requests WHERE email = ? ORDER BY request_date DESC LIMIT 1";
        ...
        $sql = "SELECT id, full_name FROM guesttbl WHERE email = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "INSERT INTO guest_password_reset_requests (user_id, email, status) VALUES (?, ?, 'pending')";
            $stmt = $conn->prepare($sql);
