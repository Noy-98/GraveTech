...
if (isset($_GET['user_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE guest_password_reset_requests SET status = 'approved' WHERE user_id = ?");
    $stmt->bind_param("i", $user_id); // "s" stands for string

    if ($stmt->execute()) {
        $_SESSION['success'] = "Guest User Approved successfully.";
    } else {
        $_SESSION['error'] = "Error Approving Guest user.";
    }
