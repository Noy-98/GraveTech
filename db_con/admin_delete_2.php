...
if (isset($_GET['user_id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM department_password_reset_requests WHERE user_id = ?");
    $stmt->bind_param("i", $user_id);

    if ($stmt->execute()) {
        $_SESSION['success2'] = "Department User Request deleted successfully.";
    } else {
        $_SESSION['error2'] = "Error deleting Department user request.";
    }
